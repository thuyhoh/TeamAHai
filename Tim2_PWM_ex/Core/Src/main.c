...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void TIM_SetFreq_Hz(TIM_HandleTypeDef *htim, uint32_t freq);
/* USER CODE END 0 */
...
void TIM_SetFreq_Hz(TIM_HandleTypeDef *htim, uint32_t freq)
{
	...
	uint16_t PSCtim = htim->Instance->PSC;
	uint16_t ARRtim = Ftim/((PSCtim + 1) * freq);
	__HAL_TIM_SetAutoreload(htim, ARRtim);
}
/* USER CODE END 4 */
